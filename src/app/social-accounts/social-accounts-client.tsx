...
export default function SocialAccountsClient({ connectedAccounts }: SocialAccountsClientProps) {
  ...
  useEffect(() => {
    ...
    if (success) {
      ...
      setNotification({
        type: 'success',
        message: messages[success as keyof typeof messages] || 'Account connected successfully!'
      })
    } else if (error) {
      const messages = {
        connection_failed: 'Failed to connect account. Please try again.',
        missing_parameters: 'Invalid connection request. Please try again.',
        access_denied: 'Access denied. Please grant permissions to connect your account.',
      }
      setNotification({
        type: 'error',
        message: messages[error as keyof typeof messages] || 'Connection failed. Please try again.'
      })
    }
              ...
              <CardContent>
                {isConnected ? (
                  ...
                  <div className="space-y-3">
                    ...
                    {(platform.id === 'linkedin' || platform.id === 'twitter') ? (
                      <Link href={`/api/oauth/${platform.id}`}>
